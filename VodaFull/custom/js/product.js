...
$(document).ready(function() {
	...
	$("#addProductModalBtn").unbind('click').bind('click', function() {
		...
		$("#submitProductForm").unbind('submit').bind('submit', function() {
			...
			if(productImage && productName && quantity && rate && brandName && categoryName && productStatus && vendorName) {
			...
			// submit product detail form
		$("#submitProductDetailForm").unbind('submit').bind('submit', function() {
			...
			if(brandName == "") {
				$("#brandName").closest('.center-block').after('<p class="text-danger">Product Image field is required</p>');
				$('#brandName').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#brandName").find('.text-danger').remove();
			...
			$("#createProductDetailBtn").button('loading');
				...
				$.ajax({
  	  			...
  	  			// reset the form text
						$("#submitProductDetailForm")[0].reset();
	...
	if(productId) {
		...
		$.ajax({
		  ...
		  // 		allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"]
				...
				//Vemndor Name 
				$("#editVendorName").val(response.vendor_id);
				// category name
				...
				$("#editProductForm").unbind('submit').bind('submit', function() {
					...
					if(productName && quantity && rate && brandName && categoryName && productStatus && vendorName) {
						// submit loading button
...
function paymentOrder(orderId = null) {
	if(orderId) {
		...
		$.ajax({
			url: 'php_action/fetchProductPayment.php',
			type: 'post',
			...
			success:function(response) {				
				...
				var grandTotal = response.order[0] * response.order[1] ;
				var dueAmount =  grandTotal - paidAmount;
				var vendorid = response.order[3] ;
				$("#due").val(dueAmount);				
				...
				$("#updatePaymentOrderBtn").unbind('click').bind('click', function() {
					...
					if(payAmount && paymentType ) {
						...
						$.ajax({
							...
							url: 'php_action/editProductPayment.php',
							type: 'post',
							data: {
								orderId: orderId,
								payAmount: payAmount,
